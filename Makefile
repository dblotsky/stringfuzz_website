...
$(SAMPLE_DIR): $(PROBLEM_DIR)
	...
	for suite in `ls $(PROBLEM_DIR)`; do \
		ls $(PROBLEM_DIR)/$$suite/*.smt20 | head -n 1 | xargs -I% cp % $(SAMPLE_DIR)/$$suite-first.smt20.txt; \
		ls $(PROBLEM_DIR)/$$suite/*.smt20 | tail -n 1 | xargs -I% cp % $(SAMPLE_DIR)/$$suite-last.smt20.txt; \
		ls $(PROBLEM_DIR)/$$suite/*.smt25 | head -n 1 | xargs -I% cp % $(SAMPLE_DIR)/$$suite-first.smt25.txt; \
		ls $(PROBLEM_DIR)/$$suite/*.smt25 | tail -n 1 | xargs -I% cp % $(SAMPLE_DIR)/$$suite-last.smt25.txt; \
	done
	touch $(SAMPLE_DIR)
